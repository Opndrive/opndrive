 ...
 *
 * Ordering and the recovery record live in use-delete-operations.test.tsx.
 * They are a separate file because they need the drive store to answer
 * selectors and the router to be standing somewhere, while this suite wants
 * the barest environment a delete can run in.
 */
...
const { refreshCurrentData, notifyError, removeDeletedFolder } = vi.hoisted(() => ({
...
/**
 * Standing at the root, which is the one place `useDeletedFolderCleanup`
 * treats as un-deletable - so cleanup never navigates and these tests stay
 * about the delete loop rather than about routing.
 */
const drive = {
...
beforeEach(() => {
  ...
  // An aborted delete leaves its record behind on purpose; clear it so one
  // test's interruption is not read as the next one's.
